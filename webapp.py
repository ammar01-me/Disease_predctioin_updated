...
scaler_diabetes = pickle.load(open(f'{working_dir}diabetes_scaler.pkl','rb'))
...
with st.sidebar:
    select = option_menu('Disease Prediction Using Machine_lerning',
                        ['Description','Heart_disease','Diabetes (Sugar)','Hypertension (BP)','Lung Cancer','Animal Disease'],
                        icons=['chart-left-text','suit-heart','activity','droplet-half','lungs','alexa'],default_index=0)
...
lst_Symptom_2 = df['Symptom_2'].value_counts().index
...
if select=='Animal Disease':
    ...
    image_of_animal = st.camera_input('Enter the image of the animal',width=(350))
    if st.button('Show'):
        try:
            st.image(image_of_animal)
        except:
            st.write('Permission denied')
    col1, col2 = st.columns(2)
    with col1:
        Animal_Type = st.selectbox('Select the animal',lst_animal_type)
        Breed = st.selectbox('Select the breed type',lst_breed)
        Age = st.number_input('Enter the age of animal',0,20)
        Gender = st.selectbox('Enter the gender ',['Male','Female'])
        Weight = st.number_input('Enter the weight of animal (pounds)',0,1000)
        Symptom_1 = st.selectbox('Select the Symptom_1 from following',lst_Symptom_1)
        Symptom_2 = st.selectbox('Select the Symptom_2 from following',lst_Symptom_2)
        Symptom_3 = st.selectbox('Select the Symptom_3 from following',lst_Symptom_3)
        Symptom_4 = st.selectbox('Select the Symptom_4 from following',lst_Symptom_4)
        Duration = st.number_input('From how many days animal suffaring from this problem',0,100)
        Appetite_Loss  = st.selectbox('Is your animal facing appatite loss',['Yes','No'])
    with col2:
        Vomiting = st.selectbox('Is your animal facing vomiting problem',['Yes','No'])
        Diarrhea = st.selectbox('Is your animal have Diarrhea (intestine problem)',['Yes','No'])
        Coughing = st.selectbox('Is your animal facing coughing',['Yes','No'])
        Labored_Breathing = st.selectbox('Is your animal facing Labored_Breathing(difficulty in breathing,)',['Yes','No'])
        Lameness =st.selectbox('Is your animal facing Laneness (leg or foot pain that afects their ability to move)',['Yes','No'])
        Skin_Lesions = st.selectbox('Is your animal have any type of skin lesions',['Yes','No'])
        Nasal_Discharge = st.selectbox('Is your animal showing nesal_discharge (any fluid fllowing from nostrils)',['Yes','No'])
        Eye_Discharge = st.selectbox('Is your animal showing Eye_Discharge (any fluid fllowing through eyes)',['Yes','No'])
        Body_Temperature = st.number_input('Enter the body temprature of the animal',0,100)

    
    Animal_Type = Animal_Type_enc.transform([Animal_Type])[0]
    Breed = Breed_enc.transform([Breed])[0]
    Gender = 0 if Gender=='Male' else 0
    Symptom_1 = Symptom_1_enc.transform([Symptom_1])[0]
    Symptom_2 = Symptom_2_enc.transform([Symptom_2])[0]
    Symptom_3 = Symptom_3_enc.transform([Symptom_3])[0]
    Symptom_4 = Symptom_4_enc.transform([Symptom_4])[0]
    Appetite_Loss  = 1 if Appetite_Loss=='Yes' else 0
    Vomiting = 1 if Vomiting=='Yes' else 0
    Diarrhea = 1 if Diarrhea=='Yes' else 0
    Coughing = 1 if Coughing=='Yes' else 0
    Labored_Breathing = 1 if Labored_Breathing=='Yes' else 0
    Lameness = 1 if Lameness=='Yes' else 0
    ...
    Animal_Disease_diagnosis = ''
    if st.button("Predict Disease"):
        try:
            inp5 = [Animal_Type,Breed,Age,Gender,Weight,Symptom_1,Symptom_2,Symptom_3,Symptom_4,Duration,Appetite_Loss,Vomiting,Diarrhea,Coughing,Labored_Breathing,Lameness,Skin_Lesions,Nasal_Discharge,Eye_Discharge,Body_Temperature,Heart_Rate]
            input_data5 = np.asarray(inp5)
            input_data_reshaped5 = input_data5.reshape(1,-1)
            Animal_Disease_diagnosis = animal_model.predict(input_data_reshaped5)

        except:
            st.warning('Fill all the value first')

    st.success(Animal_Disease_diagnosis)
